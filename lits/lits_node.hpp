 ...
 * Last Modified Date: 2024-03-21 09:15:12 (Beijing Time)
 *
...
#define P2I(p) (*(Item*)&p)
    ...
    Item pmss_bulk(const records& kvs, const int l, const int r, const int ccpl, const HPT* model, const PMSS* pmss);
    ...
    // Build an inner node
    template <class records>
    InnerNode* _try_rebulk_as_model_node(const records& kvs, const int l, const int r, const int ccpl, const HPT* model, const PMSS* pmss) {
        // Variables
        int size;
    ...
    inline bool sing_insert(Item& node, const str ckey, const val cval, const int ccpl) {
    ...
    inline bool cnod_insert(Item& node, const str ckey, const val cval, const HPT* hpt, const PMSS* pmss) {
    ...
    inline val sing_upsert(Item& node, const str ckey, const val cval, const int ccpl) {
        kv* old_entry = node.get_entry();
        int cmpRes = old_entry->keycmp(ckey, ccpl);
        if (cmpRes == 0) {
    ...
    inline val cnod_upsert(Item& node, const str ckey, const val cval, const HPT* hpt, const PMSS* pmss) {
        Cnode* cnode = node.get_cnode();
        if (cnode->has_room()) {
    ...
    inline bool cnod_remove(Item& node, const str ckey, const HPT* hpt, const PMSS* pmss) {
        Cnode* cnode = node.get_cnode();
