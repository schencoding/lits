 ...
 * Last Modified Date: 2024-03-21 09:14:23 (Beijing Time)
 *
...
namespace lits {
     ...
     */
    inline int udpl(const str s1, const str s2) {
        return ucpl(s1, s2) + 1;
    }
     ...
     * @return the longest distinguishing prefix length between s1, s2, and s3
     */
    inline int udpl(const std::string& s1,
                    const std::string& s2,
                    const std::string& s3) {
        // Find the longest of the two distinguishing prefix lengths between
        // s1 and s2, and between s2 and s3.
    ...
    inline double lpkl(const std::vector<std::string>& keys) {
        ...
        for (int i = 0; i < len; ++i) {
            if (i == 0)
                dkl_sum +=
                    udpl(keys[0], keys[1]);  // Prefix length of first and second elements
            else if (i == len - 1)
                dkl_sum +=
                    udpl(keys[len - 2],
