 ...
 * Last Modified Date: 2024-03-21 09:15:02 (Beijing Time)
 *
...
namespace lits {
    ...
    public:
        HPT() {
            // The table cannot be too large! (Maximum: 2GB)
         ...
         */
        inline int getPos(const str key, const int size, int gcpl, double k = 1, double b = 0) const {
            double ps = size * k;
            double c = size * b;
        ...
        inline int getPos_woGCPL(const str key, const int size, double k = 1, double b = 0) const {
            double pro = size * k;
            double cdf = size * b;
